  ...
  <div>
    <template v-if="post.flag">
      <template v-if="post.user == user">
        <!--ログイン中ユーザなら削除ボタンの表示-->
        <!--v-menu-->
        <v-menu v-model="dmenu" absolute offset-y style="max-width: 300px">
          <template v-slot:activator="{ on, attrs }">
            <v-btn icon absolute right v-bind="attrs" v-on="on">
            ...
            <v-list-item>
              <v-dialog v-model="sakujo" max-width="500px">
                ...
                <v-card>
                  ...
                  <v-card-actions>
                    <v-spacer />
                    <v-btn color="red darken-2" text @click="sakujo = false">
                      キャンセル
                    </v-btn>
                    <v-btn
                      color="blue darken-1"

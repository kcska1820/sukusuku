<template>
  ...
  >
    <template v-slot:top>
      <v-toolbar flat>
        ...
        <v-dialog v-model="dialog" max-width="500px">
          ...
          <v-form ref="BBSaddform" v-model="valid" lazy-validation>
            <v-card>
              <v-card-title>
          ...
          <v-card>
            ...
            <v-card-actions>
              ...
              <v-btn color="red darken-2" text @click="closeDelete"
                >キャンセル</v-btn
              >
              <v-btn color="blue darken-1" text @click="blindItemConfirm"
                >非表示</v-btn
              >
              <v-spacer></v-spacer>
            </v-card-actions>
        ...
        <v-dialog v-model="dialogUnblind" max-width="500px">
          <v-card>
            <v-card-title class="text-h5"
              >このスレッドを再表示しますか？</v-card-title
            >
            <h3 class="text-center">{{ editedItem.title }}</h3>
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn color="red darken-2" text @click="closeUnblind"
                >キャンセル</v-btn
              >
              <v-btn color="blue darken-1" text @click="unblindItemConfirm"
                >非表示</v-btn
              >
              <v-spacer></v-spacer>
            </v-card-actions>
    ...
    <template v-slot:[`item.flag`]="flag">
      <template v-if="flag.item.flag == '1'"> 通常 </template>
      <template v-else-if="flag.item.flag == '2'"> 凍結中 </template>
      <template v-else-if="flag.item.flag == '3'"> 非表示 </template>
    </template>
    <template v-slot:[`item.actions`]="{ item }">
      <v-btn
        ...
        color="primary"
      >
        <v-icon @click="blindItem(item)" size="2em"> mdi-eye-outline </v-icon>
      </v-btn>
      <v-btn fab small v-if="item.flag == '3'" color="gray">
        <v-icon @click="unblindItem(item)" size="2em">
          mdi-eye-off-outline
        </v-icon>
      </v-btn>
    </template>
